...
CREATE PROCEDURE recreateTables()
	BEGIN
		...
		DECLARE tables_cursor CURSOR FOR SELECT table_name FROM information_schema.tables WHERE table_schema = 'fx' AND table_type <> 'VIEW';
...
CREATE PROCEDURE createTable(tableName NVARCHAR(255))
	BEGIN
		SET @sql =  CONCAT("CREATE TABLE fx_unittests.", tableName, " LIKE fx.", tableName);
		PREPARE stmt FROM @sql;
		EXECUTE stmt;
		DEALLOCATE PREPARE stmt;
...
CREATE PROCEDURE feedData()
	BEGIN
		INSERT INTO fx_unittests.dates SELECT * FROM fx.dates;
    END //
